...
为了展示 Whitted 与 PBR 材质，`cornell_box_mixed` 变体将两个内部方盒替换为 Trimesh 生成的闭合镜面球与玻璃球。球体让理想反射、Snell 折射和 Fresnel 边缘具有清晰轮廓，避免玻璃长方体的多界面棱镜光路干扰验证。主对比图必须注明所用变体，不能暗中改变场景。
...
每个三角形顶点计算真实 Lambert 直接光，并使用倒数深度权重做透视校正 Gouraud
插值；同一平面的相邻三角形必须保持颜色连续，不能以整面常量着色暴露对角线。
...
- Form Factor 非负；
- 相机侧开放的 Cornell Box 允许能量逃逸，每行 Form Factor 之和必须位于 `[0, 1]`；只有真正封闭的场景才应近似为 1；
...
- `tqdm` 显示五种方法总进度，并为当前图像方法显示 Tile 进度、为 Radiosity 显示源 Patch 进度；两层均保留默认 ETA、速度与耗时。
- Rasterization、Ray Casting 与 Whitted 使用共享的规则网格超采样和线性盒式降采样；Path Tracing 使用独立 Monte Carlo SPP，禁止叠加该确定性采样倍数。
...
5. 正式渲染结果写入 `outputs/`，完成验证后纳入版本控制；
...
`outputs/` 与 `docs/images/` 都是受版本控制的正式结果资产，不得整体忽略。临时帧、失败结果或调试输出必须写入独立临时目录，不能混入正式结果。
...
- 无参数 `./run.sh`：加载 `params/default.toml`，生成五方法对比、收敛验证、白炉结果和 README 图片；
- `all`：生成五种方法的效果图与总对比图；
- `validate`：执行物理、数学和统计验证；
- `readme`：生成 README 所需的精选截图及图表；
- 默认正式参数为 512×512、Path Tracing 128 SPP、收敛参考图 1024 SPP、32×32 Tile；其他确定性图像方法使用 9 个子像素样本，Whitted 使用三级平滑 Icosphere，Radiosity 使用三级 Patch 细分与材质/平面隔离的顶点辐亮度插值；
- 全部默认参数只在 `params/default.toml` 维护，Shell、CLI 和 README 不复制第二套运行逻辑。
...
4. 五种方法的实际渲染截图；
5. Path Tracing 的 1/4/16 SPP 估计图、1024 SPP 参考图与 MSE 对比；
...
正式渲染图、算法对比图、SPP/MSE 曲线和验证报告必须保留在 `outputs/` 并提交到仓库，使 README 引用的证据可复查。只允许清理明确标记的临时中间文件，不得在发布后删除 README 所依赖的结果。
...
1. `.gitignore` 在任何下载和渲染产物之前建立，且不得忽略正式 `outputs/`；
2. 五种方法均有独立入口，并能渲染 Cornell Box PNG；
3. `./run.sh` 加载默认参数，生成五张方法图、总对比图、验证结果和 README 图片；
...
1. 正式依赖包括 SciPy、Numba、Trimesh、psutil 与 tqdm；Embree 保持可选加速后端；
2. README 正式截图为 512×512，主 Path Tracing 为 128 SPP；
3. `cornell_box_mixed` 使用 Trimesh 三级 Icosphere 与平滑顶点法线展示镜面和玻璃；
4. Cornell Box 由代码生成，不依赖外部下载；可选参考数据仍须记录来源与许可证。
